...
class Bavarages:
    ...
    def add_bavarages(self, name: str, count: float, price: float):
    ...
    def buy_bavarage(self, name: str, count: float):
        for bvg in self.bavarages:
            if bvg["name"] == name.title():
...
def menu():
    ...
    while True:
        ...
        print("5. Sotib olish")
        print()
        n = input("5ta menyudan birini tanlang: ")

        if n == "1":
            name = input("Ichimlik nomini kiriting: ")
            count = float(input("Sonini kiriting: "))
